...
**Infrastructure:** `SupabaseRESTClient` (`actor`, `static let shared`) is the shared PostgREST transport. It executes `GET` requests and `callRPC` (POST `/rpc/<name>`) with `apikey` + `Bearer` headers, snake_case → camelCase JSON decoding, and typed `SupabaseRESTError`. An `accessToken: String?` property (settable via `setAccessToken(_:)`) carries the user JWT when available; falls back to the anon key for public reads. POST/PATCH/DELETE planned for later phases. All domain services that hit the real DB delegate to this client rather than constructing their own `URLRequest`.
